...
//	public static final int GPS_BUFFER_REFRESH_PERIOD = 200;
	 ...
	 */
	public static final int ACCEL_MAX_AGE = 2500;
	
	/**
	 * A proxy to the GPS sensor.
	 */
	private AccelInterface accel;
	
	/**
	 * A buffer for storing previous GPS measurements that are up to GPS_MAX_AGE old.
	 */
	private Vector<AccelDataWrapper> buff = new Vector<AccelDataWrapper>();
...
//	private Vector<GPSListener> gpsListeners = new Vector<GPSListener>();
	 ...
	 */
	public AccelDataBuffer(AccelInterface accel) {
		this.accel = accel;
	}
	 ...
	 */
	public AccelDataBuffer(AccelInterface accel, FileLogger flogger) {
		this(accel);
...
//	}
	 ...
	 */
	public synchronized PlayerAccelData getCurrLoc() throws NoNewDataException {
	...
	private synchronized void removeExpiredElements() {
		for (int i=0; i < buff.size(); i++) {
			if (buff.get(i).isOlderThan(ACCEL_MAX_AGE))
				buff.remove(i);
		}
	}
...
//	
	private void log(String msg) {
		String result = "AccelDataBuffer: " + msg;
	 ...
	 * @author Chien-Liang Fok
	 */
	private class AccelDataWrapper {
		private PlayerAccelData data;
		private long timeStamp;
		
		public AccelDataWrapper(PlayerAccelData data) {
			this.data = data;
		 ...
		 */
		public PlayerAccelData getAccelData() {
			return data;
		}
