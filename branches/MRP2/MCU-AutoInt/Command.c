...
/**
 * Variables specific to the autonomous intersection.
 */
uint8_t prevIntEvent = 0;
unsigned short prevIntRange = 0xfff;
...
/**
 * This is used by the autonomous intersection demo.
 */
void Command_doAutoIntersect() {
  uint8_t outToSerial[MAX_PACKET_LEN];
	uint16_t indx = 0; // an index into the outToSerial array
	uint16_t i;
	unsigned short currIntRange = IR_getRR();
	
	/*
	unsigned short avg = (currIntRange +  prevIntRange) / 2;
	bool newEvent = FALSE;
	
	prevIntRange = currIntRange;
	
	// The following computes intersection events and only sends a message
	// to the x86 if an event occurs.
	if (avg > 3500 && avg < 4100) {
	  if (prevIntEvent != APPROACH) { // Supress duplicate events
				if (prevIntEvent == 0 || prevIntEvent == EXIT) {
					prevIntEvent = APPROACH;
				  newEvent = TRUE;
				}
	  }
    ...
    if (prevIntEvent != ENTRY) {  // Supress duplicate events
			if (prevIntEvent == APPROACH) {
				prevIntEvent = ENTRY;
			  newEvent = TRUE;
			}
    }
		...
		if (prevIntEvent != EXIT) { // Supress duplicate events
	  	if (prevIntEvent == ENTRY) {
				prevIntEvent = EXIT;
	  	  newEvent = TRUE;
			}
		}
	...
	if (newEvent == TRUE) {
	  outToSerial[indx++] = PROTEUS_BEGIN;  // Package BEGIN packet
  	outToSerial[indx++] = PROTEUS_AUTOINT_EVENT_PACKET;  // Identify data as an autonomous intersection packet
	  outToSerial[indx++] = prevIntEvent;
	  outToSerial[indx++] = PROTEUS_END;  // Package END packet 
	}*/
	
	// The following code simply sends the raw IR range data to the x86
	outToSerial[indx++] = PROTEUS_BEGIN;  // Package BEGIN packet
	outToSerial[indx++] = PROTEUS_AUTOINT_PACKET;  // Identify data as an autonomous intersection packet
	indx = saveTwoBytes(outToSerial, indx, currIntRange); // Rear Right IR data	
	outToSerial[indx++] = PROTEUS_END;  // Package END packet 
...
interrupt 9 void sendDataInterrupt(void) {
	...
	// The following code is specific to the autonomous intersection.
	if (_int9Count % 5 == 0) { // 20ms * 5 = 100ms (10Hz)
		TaskHandler_postTask(&Command_doAutoIntersect);
	}
