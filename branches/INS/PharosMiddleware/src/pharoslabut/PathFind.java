...
public class PathFind {
	...
	public final int STNDRD_MOV_COST = 10;
	public final int STNDRD_HYP_COST = 14;
	...
	public final int [] mov_cost  = {STNDRD_MOV_COST, STNDRD_HYP_COST, STNDRD_MOV_COST, STNDRD_HYP_COST,
									 STNDRD_MOV_COST, STNDRD_HYP_COST, STNDRD_MOV_COST, STNDRD_HYP_COST, 0};
	...
	{
		int x_diff, y_diff;
		x_diff = STNDRD_MOV_COST*Math.abs(srcX - destX);
		y_diff = STNDRD_MOV_COST*Math.abs(srcY - destY);
	...
	private int findMinF(int  [] f_score, int size)
	{
		int i, j = 0, min = INFIN;
	...
	public int A_path(int src_x, int src_y, int dest_x, int dest_y, int src_orient)
	{
		...
		while( !open.isEmpty() )
		{
			min = findMinF(f_score, map.map_size);	//find node with min F (highest priority) -> O(n)
	...
	private void buildPath(int src_x, int src_y, int dest_x, int dest_y)
	{
		...
		src  = (src_y)*map.grid_size + (src_x);
		dest = (dest_y)*map.grid_size + (dest_x);
