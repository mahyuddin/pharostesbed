...
public class GroundTruth2 {
	...
	private RSSItoDist rssiToDist = new RSSIToDistM15Exp9();
	 ...
	 */
	public GroundTruth2(String expDir, long timeStepSize, int numTimeSteps) {	
		this.expDir = expDir;
		...
		while (robotEnum.hasMoreElements()) {
			...
			// Only load the first three robots...
			if (currRobot.getRobotID() == 17 || currRobot.getRobotID() == 18 || currRobot.getRobotID() == 25) {
		...
		for (int t = 0; t <  getNumTimeSteps(); t++) {
            for (int i = 0; i < getNumRobots() - 1; i++) {
                for (int j = i + 1; j < getNumRobots(); j++) {
                	...
                	//RSSIToDistM15Exp9 rssi2Dist = new RSSIToDistM15Exp9();
                    pairwiseDist[i][j][t] = rssiToDist.getDist(robot1ID, robot2ID, rssi);
                }
            }
        }
	}
