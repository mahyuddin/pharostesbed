 ...
 *
 */
package playerclient2.extra;

import playerclient2.AbstractPositionDevice;
import playerclient2.PlayerClient;
...
public class HeadingControl extends PIDController {
	 ...
	 */
	private double boundCommand (double command) {// modified by liang
		if (command == 0)
			return 0;
		if (command < 0) {
	...
	public boolean setDiffHeading (double angle) {
		...
		try {
			...
			double now = transformAngle (device.getYaw ());// modified by liang
			...
			while (now != newGoal) {
				...
				double command = getCommand (now);// modified by liang
		...
		} catch(playerclient2.NoNewDataException nnde) {
			ret = false;
		}
	...
	public boolean setHeading (float angle) {
		...
		double currentAngle;
		try {
			currentAngle =  transformAngle (device.getYaw ());
		} catch(playerclient2.NoNewDataException nnde) {
			return false;
		}
